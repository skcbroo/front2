  ...
  return (
    <NavbarLayout>
      <div className="flex justify-center items-center min-h-[80vh] px-4">
        <div className="bg-[#EBF4FF] border border-[#CBD5E1] text-[#2D3748] p-6 sm:p-8 rounded-xl shadow-md w-full max-w-2xl space-y-6">
          <h1 className="text-2xl font-bold text-center text-[#1A202C] select-none cursor-default">
            Detalhes do Crédito
          </h1>

          <div className="space-y-1 text-sm">
            <p>
              <span className="font-semibold">Processo:</span> {credito.numeroProcesso || '—'}
            </p>
            <p>
              <span className="font-semibold">Expectativa de recebimento:</span>{' '}
              <span className="text-green-700 font-semibold">
                {credito.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </span>
            </p>
            <p>
              <span className="font-semibold">Valor de aquisição:</span>{' '}
          ...
          {credito.descricao && (
            <>
              ...
              <div>
                <h2 className="text-lg font-semibold text-center text-[#2B6CB0] mb-1">Descrição</h2>
                <p className="text-justify text-sm">{credito.descricao}</p>
              </div>
            </>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <p><span className="font-semibold">📦 Cotas totais:</span> {totalCotas}</p>
            <p><span className="font-semibold">{cotasDisponiveis > 0 ? '🟢' : '🔴'} Cotas disponíveis:</span> {cotasDisponiveis}</p>
            <p><span className="font-semibold">💵 Preço por cota:</span> {precoPorCota.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
            {typeof credito.cotasDoUsuario === "number" && credito.cotasDoUsuario > 0 && (
              <p className="text-blue-700 font-medium col-span-2">
          ...
          {cotasDisponiveis > 0 ? (
            <>
              ...
              <div className="space-y-2 text-sm">
                <label htmlFor="qtd" className="font-medium block">
